// scripts/main.ts
import { world, EquipmentSlot as EquipmentSlot3 } from "@minecraft/server";
...
import { EntityComponentTypes as EntityComponentTypes2, EquipmentSlot as EquipmentSlot2, ItemComponentTypes as ItemComponentTypes2, ItemStack as ItemStack2, system } from "@minecraft/server";
...
// scripts/haxelMining.ts
var HaxelVersion = 2;
var HaxelInfoProperty = `pfe:haxelInfo`;
...
var PFEHaxelMining = class {
  onUse(data) {
    ...
    system.runJob(PFEMine(BannedBlocks.concat(localBlacklist), ComponentInfo, location, data.source, data.source.dimension, data.itemStack.getComponent(ItemComponentTypes2.Enchantable).hasEnchantment(MinecraftEnchantmentTypes.SilkTouch), data.itemStack));
  }
};
...
function PFEHaxelConfigMenu(data) {
  let dynamicProperty = data.itemStack?.getDynamicProperty(HaxelInfoProperty);
  dynamicProperty = JSON.parse(dynamicProperty);
  let Ui = new ActionFormData().title({ translate: `translation.poke_pha:haxelConfig.mainMenu.title`, with: { rawtext: [{ translate: `item.${data.itemStack?.typeId}`.replace(`\xA79PFE\xA7r`, ``) }] } }).button({ translate: `translation.poke_pha:haxelConfig.mainMenu.blacklistAdd` }, `textures/poke/common/blacklist_add`);
...
function PFEHaxelConfigBlackListAdd(data, dynamicProperty) {
  let Ui = new ModalFormData().title({ translate: `translation.poke_pha:haxelConfig.mainMenu.title`, with: { rawtext: [{ translate: `item.${data.itemStack?.typeId}`.replace(`\xA79PFE\xA7r`, ``) }] } }).textField({ translate: `translation.poke_pha:haxelConfig.blacklistAdd.textLabel` }, "", "").submitButton({ translate: `translation.poke_pha:haxelConfig.blacklistAdd.submit` });
  Ui.show(data.source).then((response) => {
    if (response.canceled)
      return;
    ...
    if (typeof block == "string") {
      ...
      if (data.itemStack == void 0)
        return;
      PokeSaveProperty(HaxelInfoProperty, data.itemStack, JSON.stringify(newProperty), data.source, EquipmentSlot2.Mainhand);
    }
  });
}
function PFEHaxelConfigBlackListRemove(data, dynamicProperty) {
  let Ui = new ActionFormData().title({ translate: `translation.poke_pha:haxelConfig.mainMenu.blacklistRemove` });
  ...
  Ui.show(data.source).then((response) => {
    ...
    for (let i = dynamicProperty.blacklist.length; i >= -1; i--) {
      if (response.selection == i) {
        ...
        if (data.itemStack == void 0)
          return;
        PokeSaveProperty(HaxelInfoProperty, data.itemStack, JSON.stringify(dynamicProperty), data.source, EquipmentSlot2.Mainhand);
      }
    }
  });
}
...
world.beforeEvents.worldInitialize.subscribe((data) => {
  data.itemComponentRegistry.registerCustomComponent(`poke_pha:identifier`, { onUseOn(data2) {
    if (data2.source.typeId == MinecraftEntityTypes.Player) {
      data2.source.sendMessage({ translate: `translation.poke_pha:identifierMessage`, with: [data2.block.typeId] });
    }
  } });
  data.itemComponentRegistry.registerCustomComponent("poke_pha:normalMining", { onMineBlock(data2) {
    PokeDamageItemUB(data2.itemStack, void 0, data2.source, EquipmentSlot3.Mainhand);
    return;
  } });
